...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          ...
                          RobertaTokenizer, RobertaForMaskedLM, pipeline,AutoTokenizer,AutoConfig,AutoModelForMaskedLM)
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                    ...
                    for ref, gold in zip(p, eval_examples):
                        ...
                        if ref == gold.target:
                            ...
                            if ref == 'true' and gold.target == 'true':  
                                ...
                                TN += 1
                        elif ref != gold.target:
                            if ref == 'false' and gold.target == 'true':  
                                FP += 1
                ...
                if f1_score > best_f1:
                    logger.info("New Best f1_score:%s", round(np.exp(f1_score), 5))
                    logger.info("  " + "*" * 20)
                    best_f1=f1_score
                    # Save best checkpoint for best ppl
                    output_dir = os.path.join(args.output_dir, 'checkpoint-best-f1-reveal-bigvul-t5based')
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
                    output_model_file = os.path.join(output_dir, "pytorch_model_f1--reveal-bigvul-t5based.bin")
                    torch.save(model_to_save.state_dict(), output_model_file)
